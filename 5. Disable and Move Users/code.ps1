...
foreach($user in $users){
    ...
    }elseif(Get-ADUser -Identity $user.samaccountname -Properties * |Where-Object {($_.Enabled -eq $TRUE)}){ #Case 2: User is Enabled
    ...
    }elseif(Get-ADUser -Identity $user.samaccountname -Properties *|Where-Object {($_.Enabled -eq $FALSE) -and ($_.DistinguishedName -match $OU)}){
